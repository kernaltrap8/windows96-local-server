UI.DiskOperations = function(){
	...
	var w96select = UI.button();
	w96select.setProperties({
		background: UI.Assets.buttonDarkActiveScale9,
		image: Y.getImage("w96"),
		font: fontSmall,
		textAlign: "center"
	});
	w96select.onClick = browseButton.onClick;
	me.addChild(w96select);
	w96select.hide();
	...
	me.refreshList = function(type){
		...
		if (currentView == "local"){
			...
			browseButton.onClick = function(){
				W96.requestFile(function(url) {
					UI.setStatus("Loading file",true);
					Tracker.load(url, true, function() {
						UI.setStatus("Ready");
					});
				});
			};
